...
课本中“推论 2.3.1”的证明大意为：设 S1, S2 是 R^n 中的非空闭凸集，且
...
> 由 S1, S2 的闭凸性知，S 为闭凸集。

这一说法有误。虽然 S1 - S2 一定是凸集，但它未必是闭集。因此后续直接对 S1 - S2 使用需要闭性的凸集分离定理是不严谨的。
...
所以，课本证明中从“S1, S2 闭凸”推出“S1 - S2 闭凸”的步骤不成立。
...
下面给出一个不需要 S1 - S2 闭性的证明。

### 引理 1：最佳逼近的刻画

设 C 是内积空间 H 中的闭凸子集，且 y 属于 H。则 x0 属于 C 是 y 在 C 上的最佳逼近元，当且仅当

$$
\mathrm{Re}\langle y-x_0,\ x_0-x\rangle\ge 0,\qquad \forall x\in C.
$$
...
证明：反设结论不成立。则对任意非零向量 p，都存在 z 属于 C，使得
...
$$
K=\mathrm{conv}\{z_1,\ldots,z_k\}.
$$

由于 C 为凸集，故 K 包含于 C。又 K 是有限个点的凸包，因此 K 是紧凸集。
...
所以 p 不属于任何 U_zi，这与有限子覆盖矛盾。因此只能有

$$
0\in K.
$$

但 K 包含于 C，于是 0 属于 C，又与假设矛盾。故反设不成立，引理得证。

### 推论：非空凸集的弱分离

设 S1, S2 是 R^n 中的非空凸集，且
...
于是对任意 x 属于 S1、y 属于 S2，有 x - y 属于 C，从而
...
1. 将“闭凸且不相交即可严格分离”改为“非空凸且不相交即可弱分离”。
2. 删除或修改“由 S1, S2 的闭凸性知，S1 - S2 为闭凸集”这一错误表述。
3. 若保留严格分离结论，应额外加入保证正间隔的条件，例如
